...
| Create data ingestion| POST `/v1/app/user_data/ingestions` |建立一個上傳任務，API 的 body 有一個參數 type ，要填 `user_profiles` 或`tagging` 或 `orders` 來決定要上傳的資料類型，接著您必須自行完成上傳 CSV 的步驟，將檔案上傳到 `put_url`|
...
| Get details of data ingestion|GET `/v1/app/user_data/ingestions/{id}`| 取得 Ingestion 目前狀態與 meta data，status 值包含 `created`, `uploaded`, `validating`, `validated`, `processing`, `succeeded`, `failed`|
| Get CSV of data ingestion|GET `/v1/app/user_data/ingestions/csv?type={type}`| 取得 Ingestion 的 CSV 檔案下載連結，URL 中的參數 type 可填 `user_profiles` 或`tagging` 或 `orders`，回傳的 `downloadURL` 就是 CSV 檔案的下載連結  |
